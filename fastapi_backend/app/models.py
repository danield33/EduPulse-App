...
class Video(Base):
    ...
    lesson_links: Mapped[List["LessonVideo"]] = relationship(
        "LessonVideo", back_populates="video", cascade="all, delete-orphan"
    )
...
class Breakpoint(Base):
    ...
    lesson_video_id: Mapped[uuid.UUID] = mapped_column(
    ...
    lesson_video: Mapped["LessonVideo"] = relationship(
        "LessonVideo", back_populates="breakpoints"
    )

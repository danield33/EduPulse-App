...
async def generate_video(
    ...
    # image_data = await generate_image(request.images[0])
    ...
    # audio_data = await synthesize_speech(request.audio[0])
    ...
    (tts_req, img_req) = [audio_data, image_data]

    # --- Decode audio base64 (assuming mp3) ---
    audio_bytes = safe_b64decode(tts_req)
    audio_path = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3").name
    with open(audio_path, "wb") as f:
        f.write(audio_bytes)

    # --- Decode image base64 (assuming jpg/png) ---
    image_bytes = safe_b64decode(img_req)
